...
import { PROVIDER_CORRECTIONS, correctionsByEmail } from './corrections'
import { isTestBooking, isTestPackageTemplate } from './exclusions'
...
async function main() {
  ...
  // ---- exclusions, applied once, at the door ----
  //
  // Filtered here rather than at each insert, so nothing further down has to remember. The
  // cascades matter as much as the rule: a checkout link without its booking is a payment page
  // for an appointment that does not exist, and a redemption without one is a session deducted
  // from a package for a treatment nobody had.
  //
  // Every count is printed. The previous loader dropped $1,632 of package transactions in
  // silence — the right outcome, chosen by nobody, recorded nowhere.
  const droppedBookings = xBookings.filter((b) => isTestBooking(b.client_name))
  const keptBookings = xBookings.filter((b) => !isTestBooking(b.client_name))
  const droppedBookingIds = new Set(droppedBookings.map((b) => b.id))

  const droppedTemplates = xPackageTemplates.filter((t) => isTestPackageTemplate(t.name))
  const droppedTemplateIds = new Set(droppedTemplates.map((t) => t.id))
  const keptTemplates = xPackageTemplates.filter((t) => !droppedTemplateIds.has(t.id))

  const droppedPackages = xClientPackages.filter((p) =>
    droppedTemplateIds.has(p.package_template_id),
  )
  const droppedPackageIds = new Set(droppedPackages.map((p) => p.id))
  const keptPackages = xClientPackages.filter((p) => !droppedPackageIds.has(p.id))

  const keptTemplateItems = xPackageTemplateItems.filter(
    (i) => !droppedTemplateIds.has(i.package_template_id),
  )
  const keptPackageItems = xClientPackageItems.filter(
    (i) => !droppedPackageIds.has(i.client_package_id),
  )
  const keptRedemptions = xRedemptions.filter(
    (r) =>
      !(r.booking_id && droppedBookingIds.has(r.booking_id)) &&
      !(r.client_package_id && droppedPackageIds.has(r.client_package_id)),
  )
  const keptCheckoutLinks = xCheckoutLinks.filter((l) => !droppedBookingIds.has(l.booking_id))
  const keptPackageTransactions = xPackageTransactions.filter(
    (t) => !droppedTemplateIds.has(t.package_template_id),
  )

  console.log('')
  console.log('  excluded, per scripts/etl/exclusions.ts:')
  console.log(`    ${droppedBookings.length} test bookings, ${xCheckoutLinks.length - keptCheckoutLinks.length} checkout links`)
  console.log(`    ${droppedTemplates.length} package template(s), ${droppedPackages.length} packages, ` +
    `${xClientPackageItems.length - keptPackageItems.length} items, ${xRedemptions.length - keptRedemptions.length} redemptions`)
  console.log(`    ${xPackageTransactions.length - keptPackageTransactions.length} package transactions`)
  console.log(`  keeping ${keptBookings.length} bookings`)
  console.log('')
  /** Provider id -> membership id, read back after the memberships are inserted.
   *
   *  Read from the database rather than carried through in memory: the ids are generated by
   *  Postgres, so the rows going in do not know what they will be called. */
  const membershipIdByProvider = async () => {
    const rows = await db
      .select({ id: schema.memberships.id, providerId: schema.memberships.providerId })
      .from(schema.memberships)
    return new Map(rows.map((r) => [r.providerId, r.id]))
  }
  ...
  // ---- providers ----
  // Corrections applied here rather than by hand afterwards. A manual fix is invisible —
  // verify-providers.ts compares the database to Xano, so a deliberate correction and a
  // transform bug produce the identical symptom — and it would need redoing for production and
  // again after every copy-down.
  const providerRows = xProviders.map(T.transformProvider).map((row) => {
    const correction = correctionsByEmail.get(String(row.email).toLowerCase())
    if (!correction) return row

    return {
      ...row,
      ...('stripeAccountId' in correction.set ? { stripeAccountId: null } : {}),
      ...('stripeOnboardingComplete' in correction.set
        ? { stripeOnboardingComplete: correction.set.stripeOnboardingComplete }
        : {}),
      ...('bookingEnabled' in correction.set
        ? { bookingEnabled: correction.set.bookingEnabled }
        : {}),
    }
  })

  if (PROVIDER_CORRECTIONS.length) {
    console.log(`  ${PROVIDER_CORRECTIONS.length} provider correction(s) applied:`)
  ...
  // ---- clients (derived) ----
  // Derived from what actually survives. Built from the raw lists, a dropped test booking
  // would still create "Test Test" as a real client record.
  const { rows: clientRows, byKey } = T.buildClients(keptBookings, keptPackages)
  if (clientRows.length) await db.insert(schema.clients).values(clientRows)

  // ---- bookings ----
  const redemptionBookingIds = new Set(keptRedemptions.map((r) => r.booking_id))
  const paidCheckoutBookingIds = new Set(
    keptCheckoutLinks.filter((c) => c.status === 'paid').map((c) => c.booking_id),
  )

  // Two filters, two different questions: keptBookings excludes test data, this excludes
  // bookings whose provider did not make it across.
  const bookableBookings = keptBookings.filter((b) => liveProviders.has(b.provider_id))
  const bookingRows = bookableBookings.map((b) => {
  ...
  const checkoutRows = keptCheckoutLinks
  ...
  const templateRows = keptTemplates
  ...
  const templateItemRows = keptTemplateItems
  ...
  const clientPackageRows = keptPackages
  ...
  const packageItemRows = keptPackageItems
  ...
  const redemptionRows = keptRedemptions
    .filter(
      (r) =>
  ...
  const tipByCheckoutLinkId = new Map(
    keptCheckoutLinks.map((c) => [c.id, Number(c.tip_amount ?? 0)]),
  )
  ...
  const candidate = [
    ...
    ...T.ledgerFromPackageTransactions(keptPackageTransactions, new Map()),
    ...T.ledgerFromRoomTransactions(xRoomTransactions),
    // Built from the rows just inserted, not left empty. Passing `new Map()` here is what made
    // every membership ledger entry point at a provider instead of a membership.
    ...T.ledgerFromStripeInvoices(sInvoices, await membershipIdByProvider()),
