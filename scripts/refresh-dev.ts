...
async function main() {
  ...
  const script = [
    ...
    dump,
    '',
    "-- pg_dump writes `set_config('search_path', '', false)` and then schema-qualifies every",
    '-- name it emits. That empty search_path outlives its own section, so the unqualified',
    '-- `UPDATE clients` below resolves to nothing — "relation clients does not exist", on a',
    '-- table that is sitting right there, freshly populated. Put it back before the scrub.',
